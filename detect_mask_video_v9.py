...
def getinfo():  # Função que abre comunicação serial com o microcontrolador e recebe as medições
    ...
    while (
            str_data != 'f'):  # Loop para leitura e arquivo das medições, até que receba o caractere f que sinaliza o fim do envio de medições
        data = arduino.readline()  # Lê o conteúdo da porta serial e aloca na variável data
        str_data = data.rstrip().decode('utf-8')  # Decodifica o dado hexadecimal e aloca na variável str_data
        if str_data != 'f': oxi_l.append(
            str_data)  # Guarda o valor recebido em strdata como um novo item na lista temp_l
...
def pix():
    ...
    while True:
        ...
        while flag_info == 0:  # Loop que envia o caracter t ao microcontrolador solicitando as medições de temperatura
            ...
            print('entrou na pix e abriu serial')
            str_data = ''
            while str_data != 'f' and str_data != 'p':
                ...
                print('caracter p enviado')
                time.sleep(0.5)  # Aguarda 0,1 segundos
                data = arduino.readline()  # Lê o conteúdo da porta serial e aloca na variável data
                str_data = data.rstrip().decode('utf-8')  # Decodifica o dado hexadecimal e aloca na variável str_data
                print(f'recebido {str_data}')
                if str_data == 'p':
                    sensor = 1
                    arduino.close()  # Encerra a comunicação serial com o microcontrolador
                    print('setou sensor = 1 e fechou serial')
                elif str_data == 'f':
                    sensor = 0
                    arduino.close()  # Encerra a comunicação serial com o microcontrolador
                    print('setou sensor = 0 e fechou serial')
...
time.sleep(2.0)
threading.Thread(target=pix).start()  # Executa a função que solicita as medidas
time.sleep(2.0)
...
while True:
    ...
    for (box, pred) in zip(locs, preds):
        ...
        if label == "Sem Mascara":
            ...
            color6 = (0, 255, 255)
            org6 = (260, 225)
            cv2.putText(frame, label6, org6, cv2.FONT_HERSHEY_TRIPLEX, 0.45, color6, 2)
        #if label == "Com Mascara"  and chave == 'liberado:'
        if sensor == 1 and flag_info == 0: # Se há uma alteração no estado de Sem Máscara para Com Máscara
            threading.Thread(target=measures).start()
            print("chamou measures")
        #    print('rodouthread')
        ...
        if ox1 != old_ox1:  # Quando as novas medidas são atualizadas
            ...
            if label == "Com Mascara" and float(temp1) < 37.5 and float(ox1) > 89:  # Se o usuário está de máscara, possui temperatura e oxigenação dentro da faixa aceitável
                cv2.putText(frame, label4, org4,
                            cv2.FONT_HERSHEY_TRIPLEX, 0.45, color, 2)  # Exibe entrada autorizada
            else:
                cv2.putText(frame, label5, org5,
                            cv2.FONT_HERSHEY_TRIPLEX, 0.45, color, 2)  # Se não, exibe entrada não autorizada
